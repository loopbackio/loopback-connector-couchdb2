...
CouchDB.prototype.autoupdate = function(models, cb) {
  ...
  function autoUpdateOneModel(model, cb1) {
    var mo = self.selectModel(model, true);
    var indexes = mo.mo.model.definition.indexes();
    var modelView = mo.modelView;

    var modelLevelIndexes = _.pickBy(indexes, function(value, index) {
      if (value.keys)
        return indexes[index];
    });

    var propertyLevelIndexes = _.pickBy(indexes, function(value, index) {
      if (!value.keys)
        return indexes[index];
    });

    async.series([
      function prepPropertyLevelIndex(cb2) {
        createPropertyLevelIndex(propertyLevelIndexes, cb2);
      },
      function prepModelLevelIndexes(cb3) {
        createModelLevelIndexes(modelLevelIndexes, cb3);
      },
    ], cb1);

    function createPropertyLevelIndex(indexes, cb4) {
      async.eachOf(indexes, function(value, index, cb5) {
        var fields = [index.split('_index')[0]];
        createIndex(index, fields, cb5);
      }, function(err) {
        cb4(err);
      });
    }

    function createModelLevelIndexes(indexes, cb6) {
      async.eachOf(indexes, function(value, index, cb7) {
        var keys = value.keys;
        var fields = [];
        ...
        createIndex(index, fields, cb7);
      }, function(err) {
        cb6(err);
      });
    }

    function createIndex(name, fields, cb8) {
      var config = {
        ddocName: modelView + '__' + model + '__index__' + name,
        indexName: name,
        fields: fields,
      };
      self.createIndex(config.ddocName, config.indexName, config.fields, cb8);
    }
  };
};
 ...
 */
CouchDB.prototype.getIndexes = function(cb) {
  var self = this;
  var requestObject = {
    db: self.settings.database,
    path: '_index',
