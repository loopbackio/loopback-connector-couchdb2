...
CouchDB.prototype.autoupdate = function(models, cb) {
  ...
  function autoUpdateOneModel(model, callback) {
    ...
    var indexes = mo.mo.settings.indexes;
    var modelView = mo.modelView;

    async.series([
      function propertyLevelIndex(callback) {
        createPropertyLevelIndex(properties, callback);
      },
      function modelLevelIndex(callback) {
        createModelLevelIndex(indexes, callback);
      },
    ], cb);

    function createPropertyLevelIndex(properties, callback) {
    ...
    function createModelLevelIndex(indexes, callback) {
      async.eachOf(indexes, function(keys, name, cb) {
        debug('automigrate iterate indexes: keys %s, name %s',
              keys.keys, name);
        var fields = [];
        var keys = keys.keys;
        var order;
        ...
        createIndex(name, fields, cb);
      }, function(err) {
        callback(err);
      });
    };

    function createIndex(props, fields, cb) {
      var fields = Array.isArray(fields) ? fields : [fields];
      var config = {
        ddocName: modelView + '__' + model + '__property__' + props,
        indexName: modelView + '__' + model + '__property__' + props,
        fields: fields,
      };
