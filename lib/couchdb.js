  ...
  for (var i = 0; i < modelObject.dateFields.length; i++) {
    ...
    if (dateValue) {
      if (Array.isArray(dateValue)) {
        doc[dateField] = dateValue.map(function(dateValue) { return new Date(dateValue)})
      } else doc[dateField] = new Date(dateValue);
    }  
  }
...
CouchDB.prototype.selectModel = function(model, migrate) {
  ...
  for (var p in mo.properties) {
    ...
    if (Array.isArray(mo.properties[p].type) && mo.properties[p].type.length>0) {
      if (mo.properties[p].type[0].name === 'Date') {
        dateFields.push(p);
      }
    }
  }
