...
const Driver = require('nano');
const assert = require('assert');
const debug = require('debug')('loopback:connector:couchdb2');
...
CouchDB.prototype.toDB = function(modelName, modelObject, doc) {
  ...
  let idValue = this.getIdValue(modelName, doc);
...
CouchDB.prototype.fromDB = function(modelName, modelObject, doc, fields) {
  ...
  if (idName && modelObject.mo.properties[idName])
    idType = modelObject.mo.properties[idName].type.name;
  if (!doc) return doc;
  assert(doc._id);
  ...
  for (let i = 0; i < modelObject.dateFields.length; i++) {
    const dateField = modelObject.dateFields[i];
...
CouchDB.prototype._insert = function(model, data, cb) {
  ...
  mo.db.insert(self.toDB(model, mo, data), function(err, result) {
    ...
    if (mo.mo.properties[idName]) {
      const idType = mo.mo.properties[idName].type.name;
      if (idType === 'Number') {
...
CouchDB.prototype.getCurrentRevision = function(model, id, cb) {
  const mo = this.selectModel(model);
...
CouchDB.prototype.buildSort = function(mo, model, order) {
  ...
  let field, fieldType, nestedFields, obj;
  const sort = [];
  const props = mo.mo.properties;
  ...
  for (const i in order) {
    const k = order[i];
    const m = k.match(/\s+(A|DE)SC$/);
    let n = k.replace(/\s+(A|DE)SC$/, '').trim();
    obj = {};
...
CouchDB.prototype.updateOrCreate = function(model, data, cb) {
  ...
  const idName = self.idName(model);
  const mo = self.selectModel(model);
  const id = data[idName].toString();

  // Callback handler for both create calls.
  const createHandler = function(err, id) {
    if (err) return cb(err);
    mo.db.get(id, function(err, doc) {
      if (err) return cb(err);
    ...
    self.all(model, {where: where}, {raw: true}, function(err, docs) {
      ...
      async.each(docs, function(doc, cb) {
        ...
        mo.db.bulk({docs: docs}, function(err, result) {
          if (err) return cb(err);
          const errorArray = _.filter(result, 'error');
...
CouchDB.prototype.bulkReplace = function(model, dataList, cb) {
  ...
  const dataToBeUpdated = _.map(dataList, function(data) {
...
CouchDB.prototype.replaceById = function(model, id, data, options, cb) {
  ...
  const replaceHandler = function(err, id) {
    if (err) return cb(err);
    mo.db.get(id, function(err, doc) {
      if (err) return cb(err);
      cb(null, self.fromDB(model, mo, doc));
...
CouchDB.prototype.selectModel = function(model, migrate) {
  const self = this;
  let dbName, partitionKey;
  let modelView = null;
  let modelSelector = null;
  const dateFields = [];
  const s = this.settings;

  const db = this.pool[model];
  if (db && !migrate) return db;

  const mo = this._models[model];
  const dbSettings = self.getModelObjectSettings(mo);
...
CouchDB.prototype.updateIndex = function(mo, modelName, cb) {
  ...
  /* eslint-enable camelcase */
  const indexView = util.inspect(idx, 4);
...
CouchDB.prototype._buildQuery = function(model, idName, query, where) {
  ...
  Object.keys(where).forEach(function(k) {
    ...
    if (spec) {
      const selectedOperator = self._selectOperator(spec, cond, containsRegex);
      query[k] = selectedOperator[0];
      containsRegex = selectedOperator[1];
    } else query[k] = cond;

    const filterWithArray = self._buildFilterArr(k, mo.mo.properties);
    ...
    if (typeof filterWithArray === 'string') {
      const kParser = filterWithArray.split('.');
      if (kParser.length > 1) {
...
CouchDB.prototype._regexToPCRE = function(regex, negative) {
  ...
  const flags = regex.flags ? '(?' + regex.flags + ')' : '';
...
CouchDB.prototype._buildFilterArr = function(k, props) {
  ...
  const newFields = [];
  let currentProperty = props;
  let field = '';
  let propIsArr = false;
  let propIsObjWithTypeArr = false;
  ...
  for (let i = 0; i < len; i++) {
    if (propIsArr) {
      ...
      // }
      field = null;
      for (const property in currentProperty) {
...
CouchDB.prototype._extendDocs = function(rst, docs, query, mo, include, options, cb) {
  ...
  for (let i = 0; i < rst.docs.length; i++) {
...
CouchDB.prototype.getIdName = function(model) {
  const self = this;
  const cachedModel = self.pool[model];
