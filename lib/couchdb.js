...
/**
 * Called by `automigrate` with isMigrate === true 
 * or `autoupdate` with isMigrate === false
 */
...
CouchDB.prototype.migrateOrUpdateIndex = function(models, isMigrate, cb) {
  ...
  function autoUpdateOneModel(model, cb) {
    ...
    function createIndex(name, fields, cb) {
      ...
      var config = {
        ddocName: modelView + '__' + model + '__LBIndex__' + name,
        indexName: name,
        fields: fields,
      };
...
/**
 * Default as 'LBModel', but now is still 'loopback__model__name'
 */
