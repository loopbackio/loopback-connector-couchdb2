...
function mixinMigrate(CouchDB) {
  const debug = require('debug')('loopback:connector:couchdb2:migrate');
 ...
 */
  ...
  CouchDB.prototype.migrateOrUpdateIndex = function(models, isMigrate, cb) {
    ...
    function autoUpdateOneModel(model, cb) {
      ...
      const mo = self.selectModel(model, true);
      if (!mo) return cb(new Error('model ' + model + ' does not exist in your registry!'));
      const indexes = mo.mo.model.definition.indexes();
 ...
 */
  CouchDB.prototype.coerceIndexFields = function(fields) {
    ...
    const defaultOrder = this.getDefaultOrder(fields);
    const coercedFields = [];

    const result = _.map(fields, coerceOrder);
    ...
    function printWarning() {
      const couchRule = 'Couchdb does NOT allow composite indexes with conflicting sort directions, ' +
        'please see http://docs.couchdb.org/en/2.0.0/api/database/find.html#db-index for details.' + '\n';
      const connectorStrategy = 'The index will be created using ' + defaultOrder + ' order, specified by: ';
 ...
 */
  CouchDB.prototype.getModifyIndexes = function(mo, newLBIndexes, isMigrate, cb) {
    ...
    // store in `newIndexes`
    let newIndexes = {};
    const modelName = mo.mo.model.modelName;

    const newModelIndexes = _.pickBy(newLBIndexes, function(value, index) {
      if (value.keys)
        return newLBIndexes[index];
    });
    const newPropertyIndexes = _.pickBy(newLBIndexes, function(value, index) {
      if (!value.keys)
        return newLBIndexes[index];
    });
 ...
 */
  CouchDB.prototype.getModelIndexes = function(model, cb) {
    ...
    self.getIndexes(dbName, function(err, result) {
      ...
      const existingIndexes = self.parseIndexes(ddocsOfModel);
